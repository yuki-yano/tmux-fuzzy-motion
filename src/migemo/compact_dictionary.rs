    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let next_values = self.key_node_indices.find_map(|node_index| {
                if node_index < dict.has_mapping_bit_list.len()
                    && dict.has_mapping_bit_list.get(node_index)
                {
...
mod tests {
    ...
    #[test]
    fn test_1() {
        let buf = include_bytes!("../../assets/migemo-compact-dict").to_vec();
        let dict = CompactDictionary::new(&buf);
        let word: Vec<u16> = "けんさく".encode_utf16().collect();
        let words = dict
            .search(&word)
            .map(|word| String::from_utf16_lossy(&word).to_string())
            .collect::<Vec<_>>();
        assert!(words.contains(&"検索".to_string()));
    }
}
