...
pub fn build(mut dict: HashMap<String, Vec<String>>) -> Vec<u8> {
    ...
    let mut values: Vec<Vec<u16>> = values_set
        .iter()
    ...
    // output key trie
    output_data
        .write_i32::<BigEndian>(key_trie.edges.len() as i32)
        .unwrap();
    ...
    for i in 0..mapping_words_len {
        output_data
            .write_u64::<BigEndian>(mapping_bit_list.words()[i])
            .unwrap();
    }
    output_data
        .write_i32::<BigEndian>(mapping.len() as i32)
        .unwrap();
...
mod tests {
    ...
    fn test_1() {
        ...
        let expected_buffer: Vec<u8> = vec![
            0x00, 0x00, 0x00, 0x06, 0x20, 0x20, 0xB1, 0xF3, 0xB5, 0xAF, 0x00, 0x00, 0x00, 0x0A,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x55, 0x00, 0x00, 0x00, 0x07, 0x00, 0x20,
            0x00, 0x20, 0x69, 0x1C, 0x78, 0x14, 0x67, 0xFB, 0x7D, 0x22, 0x52, 0x4A, 0x00, 0x00,
            0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x6D, 0x00, 0x00, 0x00, 0x08,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xD0, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00,
